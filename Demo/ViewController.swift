...
//		appearance.textFont = UIFont(name: "Georgia", size: 14)
		...
		dropDowns.forEach {
			...
			$0.customCellConfiguration = { (index: Index, _: String, cell: DropDownCell) -> Void in
	...
	func setupChooseArticleDropDown() {
		...
		chooseArticleDropDown.selectionAction = { [weak self] (_, item) in
			self?.chooseArticleButton.setTitle(item, for: .normal)
		}

        chooseArticleDropDown.multiSelectionAction = { [weak self] (_, items) in
	...
	func setupAmountDropDown() {
		...
		// Action triggered on selection
		amountDropDown.selectionAction = { [weak self] (_, item) in
	...
	func setupChooseDropDown() {
		...
		// Action triggered on selection
		chooseDropDown.selectionAction = { [weak self] (_, item) in
...
//		centeredDropDown.anchorView = centeredDropDownButton
        ...
        centeredDropDown.selectionAction = { [weak self] (_, item) in
